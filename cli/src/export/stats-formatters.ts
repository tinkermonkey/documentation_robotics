...
function formatText(stats: ModelStats, verbose?: boolean): string {
  ...
  lines.push(
    ansis.bold(
      `Total: ${ansis.cyan(`${stats.statistics.totalElements} elements`)} across ${ansis.cyan(`${stats.statistics.totalLayers} layers`)}`
    )
  );
  ...
  if (stats.statistics.totalRelationships > 0) {
    ...
    lines.push("");
    lines.push(
      ansis.bold(`Total Relationships: ${ansis.cyan(stats.statistics.totalRelationships)}`)
    );
  ...
  if (verbose || stats.orphanedElements.length > 0) {
    ...
    if (stats.orphanedElements.length === 0) {
      ...
      lines.push(
        `  ${ansis.yellow(`⚠`)} ${stats.orphanedElements.length} orphaned element${stats.orphanedElements.length > 1 ? "s" : ""}`
      );
      if (verbose) {
...
function formatMarkdown(stats: ModelStats): string {
  ...
  if (stats.orphanedElements.length === 0) {
    ...
    lines.push(
      `- ⚠ ${stats.orphanedElements.length} orphaned element${stats.orphanedElements.length > 1 ? "s" : ""}`
    );
  }
  lines.push("");
