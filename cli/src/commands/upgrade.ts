...
/**
 * Read CLI version from the CLI's own package.json
 */
async function getPackageVersion(): Promise<string> {
  // Only check the CLI's package.json, not the project's package.json
  const cliPackagePath = `${import.meta.url.replace('file://', '').split('/src/')[0]}/package.json`;

  if (await fileExists(cliPackagePath)) {
    try {
      const data = await readJSON<{ version: string }>(cliPackagePath);
      return data.version;
    } catch {
      // Fall back to hardcoded version if we can't read package.json
    }
  }
