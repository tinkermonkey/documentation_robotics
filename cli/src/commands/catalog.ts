...
Examples:
  ...
  // catalog info - Show catalog metadata or predicate details
  catalog
    .command("info [predicate]")
    .description("Show relationship catalog metadata or predicate details")
    .addHelpText(
      "after",
      `
Examples:
  $ dr catalog info
  $ dr catalog info composes
  $ dr catalog info implements`
    )
    .action(async (predicate?: string) => {
      try {
        const catalogInstance = new RelationshipCatalog();
        await catalogInstance.load();

        if (predicate) {
          // Show specific predicate details
          const type = catalogInstance.getTypeByPredicate(predicate) || catalogInstance.getTypeById(predicate);

          if (!type) {
            console.error(ansis.red(`Unknown predicate: ${predicate}`));
            console.error(ansis.dim("Use `dr catalog types` to list valid predicates"));
            process.exit(1);
          }

          console.log("");
          console.log(ansis.bold(ansis.blue(`Predicate: ${type.predicate}`)));
          console.log(ansis.dim("─".repeat(60)));
          console.log("");
          console.log(`${ansis.bold("ID:")}               ${type.id}`);
          console.log(`${ansis.bold("Predicate:")}        ${ansis.cyan(type.predicate)}`);
          if (type.inversePredicate) {
            console.log(`${ansis.bold("Inverse:")}          ${ansis.cyan(type.inversePredicate)}`);
          }
          console.log(`${ansis.bold("Category:")}         ${type.category}`);
          if (type.archimateAlignment) {
            console.log(`${ansis.bold("ArchiMate:")}        ${type.archimateAlignment}`);
          }
          console.log("");
          console.log(`${ansis.bold("Description:")}      ${type.description}`);
          console.log("");
          console.log(ansis.bold("Semantics:"));
          console.log(`  Directionality: ${type.semantics.directionality}`);
          console.log(`  Transitivity:   ${type.semantics.transitivity ? "yes" : "no"}`);
          console.log(`  Symmetry:       ${type.semantics.symmetry ? "yes" : "no"}`);
          if (type.semantics.reflexivity !== undefined) {
            console.log(`  Reflexivity:    ${type.semantics.reflexivity ? "yes" : "no"}`);
          }
          if (type.applicableLayers.length > 0) {
            console.log("");
            console.log(`${ansis.bold("Applicable Layers:")} ${type.applicableLayers.join(", ")}`);
          }
        } else {
