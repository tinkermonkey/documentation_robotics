...
import { listChatSessions, readChatSession, getChatLogger, ChatLogEntry } from '../utils/chat-logger.js';
...
export function chatLogsCommand(program: Command): void {
  const logsCommand = program
    .command('chat-logs')
    .description('View and manage chat session logs');
  ...
  $ dr chat-logs list --limit 20      # Show last 20 sessions`
    ...
    .action(async (options) => {
      try {
        ...
        console.log(
          ansis.dim(
            `Use 'dr chat-logs view <session-id>' to view a session`
          )
        );
        console.log(
          ansis.dim(`Total sessions: ${sessions.length}`)
        );
        console.log('');
      } catch (error) {
  ...
  $ dr chat-logs view <session-id> --no-truncate       # Show full content`
    ...
    .action(async (sessionFile, options) => {
      try {
        ...
        // Find session info
        const sessionStart = entries.find((e) => e.type === 'event' && e.content === 'Session started');
  ...
  $ dr chat-logs info    # Show logs directory path and stats`
    ...
    .action(async () => {
      try {
        ...
        const projectLogDir = projectRoot
          ? join(projectRoot, '.dr', 'chat', 'sessions')
          : null;
