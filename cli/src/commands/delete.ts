  ...
  try {
    ...
    if (dependents.length > 0) {
      ...
      if (!options.cascade && !options.force) {
        ...
        throw new CLIError('Element has dependencies', 1);
      }
    }
    ...
    if (options.cascade && dependents.length > 0) {
      ...
      // We need to delete in reverse topological order, but for simplicity,
      // we'll delete all dependents first, then the target element
