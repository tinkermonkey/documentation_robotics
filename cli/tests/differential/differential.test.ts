...
import { type TestCase, executeTestCase, formatCommand, formatCommandResult } from "./runner";
import { compareOutputs, formatComparisonResult } from "./comparator";
...
describe("Differential Testing: Python CLI vs TypeScript CLI", () => {
  ...
  for (const testCase of testCases) {
    ...
    testFn(
      ...
      120000
    ); // 2 minute timeout for each test
  }
});
  ...
  test("Generate compatibility report", async () => {
    ...
    for (const [category, cases] of categories) {
      ...
      for (const tc of cases) {
        const status = tc.skip ? "⊘ SKIP" : tc.expectedToFail ? "⚠ EXPECT FAIL" : "○ RUN";
