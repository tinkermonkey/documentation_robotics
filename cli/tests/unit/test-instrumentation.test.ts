...
describe("Test Instrumentation (Production No-Op Mode)", () => {
  describe("startTestFileSpan()", () => {
    it("should return null when TELEMETRY_ENABLED is false", () => {
      const span = testInstrumentation.startTestFileSpan("tests/unit/example.test.ts");
    ...
    it("should not throw with absolute file path", () => {
      expect(() => {
        testInstrumentation.startTestFileSpan("/absolute/path/to/test.ts");
      }).not.toThrow();
    });

    it("should not throw with relative file path", () => {
      expect(() => {
        testInstrumentation.startTestFileSpan("../relative/path/to/test.ts");
      }).not.toThrow();
    });
  });
  ...
  describe("createTestCaseSpan()", () => {
    ...
    it("should return null with test name only", () => {
      const span = testInstrumentation.createTestCaseSpan("should do something");
      expect(span).toBeNull();
    });

    it("should return null with test name and suite name", () => {
      const span = testInstrumentation.createTestCaseSpan("should do something", "MyTestSuite");
      expect(span).toBeNull();
    });
  ...
  describe("instrumentTest()", () => {
    ...
    it("should accept optional suite name", () => {
      const wrapped = testInstrumentation.instrumentTest("test name", () => {}, "Suite Name");
      expect(typeof wrapped).toBe("function");
    });
  ...
  describe("Integration: File and Case Spans", () => {
    ...
    it("should handle case spans within file span", async () => {
      expect(() => {
        ...
        const span1 = testInstrumentation.createTestCaseSpan("test 1", "Suite");
        testInstrumentation.recordTestResult(span1, "pass");

        const span2 = testInstrumentation.createTestCaseSpan("test 2", "Suite");
