...
describe("ChangesetValidator - Validation on Projected Models", () => {
  ...
  it("should catch invalid element ID naming convention", async () => {
    ...
    expect(
      validation.errors.some((e) => e.message.includes("naming") || e.message.includes("ID format"))
    ).toBe(true);
  });
  ...
  it("should validate inter-stage references", async () => {
    ...
    expect(
      validation.errors.filter((e) => e.message.includes("Reference")).length
  ...
  it("should detect reference to non-existent element", async () => {
    ...
    expect(
      validation.errors.some(
        (e) => e.message.includes("Reference") || e.message.includes("non-existent")
      )
    ).toBe(true);
  });
  ...
  it("should enforce higher->lower layer reference constraint", async () => {
    ...
    expect(validation.isValid()).toBe(false);
    expect(
      validation.errors.some((e) => e.message.includes("layer") || e.message.includes("reference"))
    ).toBe(true);
  });
