...
describe("Chat JSON Event Parsing", () => {
  ...
  it("should handle mixed content blocks", () => {
    const event = JSON.parse(
      '{"type":"assistant","message":{"content":[{"type":"text","text":"Let me check..."},{"type":"tool_use","name":"Bash","input":{"command":"dr list"}}]}}'
    );
  ...
  it("should handle tool_use with complex input", () => {
    const event = JSON.parse(
      '{"type":"assistant","message":{"content":[{"type":"tool_use","name":"Read","input":{"file_path":"/path/to/file.json"}}]}}'
    );
...
You can use Read to examine model files in the .dr directory.`;
  ...
  it("should include DrBot identity", () => {
    const prompt =
      "You are DrBot, an expert conversational assistant for Documentation Robotics (DR) models.";
...
describe("With-Danger Flag", () => {
  it("should build Claude CLI command without danger flag by default", () => {
    const cmd = ["claude", "--print", "--verbose", "--output-format", "stream-json"];
