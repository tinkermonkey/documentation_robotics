...
describe("Error Message Scenarios", () => {
  ...
  describe("Exit Code Categories", () => {
    ...
    it("should return exit code 2 when model not found", async () => {
      ...
      expect(
        output.includes("No model found") ||
          output.includes("No DR project") ||
  ...
  describe("Add Command Errors", () => {
    ...
    it("should provide fuzzy match suggestions for typos", async () => {
      ...
      const hasSuggestion =
        stderr.includes("apm") || stderr.includes("api") || stderr.includes("Did you mean");
    ...
    it("should error when adding duplicate element", async () => {
      ...
      expect(result2.stderr.includes("dr show") || result2.stderr.includes("dr update")).toBe(true);
    });
    ...
    it("should include operation context in error messages", async () => {
      ...
      const stderr = result.stderr;
      expect(stderr.includes("During") || stderr.includes("add") || stderr.includes("Error:")).toBe(
        true
      );
    });
  });
  ...
  describe("Delete Command Errors", () => {
    ...
    it("should error when element not found", async () => {
      ...
      expect(result.exitCode).toBe(1); // User error
      const output = result.stdout + result.stderr;
      expect(output.includes("not found") || output.includes("Element")).toBe(true);
    });
    ...
    it("should provide helpful suggestions for not found elements", async () => {
      ...
      expect(
        output.includes("search") || output.includes("list") || output.includes("Suggestions")
      ).toBe(true);
    });
  ...
  describe("Error Message Clarity", () => {
    ...
    it("should provide actionable suggestions", async () => {
      ...
      expect(
        output.includes("not found") || output.includes("Suggestions") || output.includes("Use")
      ).toBe(true);
    });
  });
  ...
  describe("Dry-run and Recovery Guidance", () => {
    ...
    it("should support --dry-run for delete operations", async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout.includes("Dry run") || result.stdout.includes("Would remove")).toBe(
        true
      );
