...
describe("ModelReportOrchestrator Integration", () => {
  ...
  it("should use correct file naming convention", async () => {
    ...
    const apiExists = await fs
      ...
      .then(() => true)
      .catch(() => false);
    const dataModelExists = await fs
      .access(dataModelPath)
      .then(() => true)
      .catch(() => false);
    const testingExists = await fs
      .access(testingPath)
      .then(() => true)
      .catch(() => false);
  ...
  it("regenerate should skip invalid layer names with warning", async () => {
    ...
    const apiPath = path.join(reportsDir, "07-api-layer-report.md");
    const motivationPath = path.join(
      reportsDir,
    ...
    const apiExists = await fs
      ...
      .then(() => true)
      .catch(() => false);
    const motivationExists = await fs
      .access(motivationPath)
      .then(() => true)
      .catch(() => false);
...
describe("Mutation → Report Generation Integration", () => {
  ...
  it("relationship add → updates reports for both source and target layers", async () => {
    ...
    const apiReportPath = path.join(reportsDir, "07-api-layer-report.md");
    const appReportPath = path.join(
      reportsDir,
      "05-application-layer-report.md"
    );
  ...
  it("relationship delete → updates reports for both affected layers", async () => {
    ...
    // Regenerate reports after adding relationship
    await orchestrator.regenerate(new Set([sourceLayerName, targetLayerName]));

    // Get report paths
    const layerNumber1 = getLayerOrder(sourceLayerName);
    const layerNumber2 = getLayerOrder(targetLayerName);
    const reportPath1 = path.join(
      reportsDir,
      `${String(layerNumber1).padStart(2, "0")}-${sourceLayerName}-layer-report.md`
    );
    const reportPath2 = path.join(
      reportsDir,
      `${String(layerNumber2).padStart(2, "0")}-${targetLayerName}-layer-report.md`
    );

    const contentAfterAdd1 = await fs.readFile(reportPath1, "utf-8");
    const contentAfterAdd2 = await fs.readFile(reportPath2, "utf-8");

    // Now delete the relationship
    model.relationships.delete(sourceElement.id, targetElement.id, "exposes");
    await model.saveRelationships();
    await model.saveManifest();

    // Regenerate reports after deleting
    await orchestrator.regenerate(new Set([sourceLayerName, targetLayerName]));

    // Verify reports were updated after deletion
    const contentAfterDelete1 = await fs.readFile(reportPath1, "utf-8");
    const contentAfterDelete2 = await fs.readFile(reportPath2, "utf-8");

    expect(contentAfterDelete1).not.toBe(contentAfterAdd1);
    expect(contentAfterDelete2).not.toBe(contentAfterAdd2);
  });
  ...
  it("report failure does not fail the mutation", async () => {
    ...
    const originalInstanceCreator =
      ModelReportOrchestrator.prototype.regenerate;
    ...
    try {
      ...
      expect(String(lastCall[1])).toContain(
        "Failed to update layer reports after mutation"
      );
    } finally {
  ...
  it("staged change does not trigger report generation", async () => {
    ...
    const changeset = await stagingManager.create(
      "test-changeset",
      "Test staged changes"
    );
