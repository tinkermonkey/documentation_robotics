...
describe('Source Reference CLI Integration Tests', () => {
  ...
  describe('add command with source references', () => {
    it('should add element with source-file and source-provenance', async () => {
      const result = await runDr('add', 'security', 'policy', 'auth-validate',
    ...
    it('should add element with all source reference options', async () => {
      const result = await runDr('add', 'api', 'endpoint', 'create-customer',
        '--name', 'Create Customer Endpoint',
        '--source-file', 'src/api/endpoints/customer.ts',
        '--source-symbol', 'createCustomer',
        '--source-provenance', 'extracted',
        '--source-repo-remote', 'https://github.com/example/repo.git',
        '--source-repo-commit', '1234567890123456789012345678901234567890'
      );
    ...
    it('should fail when source-file is missing but other source options provided', async () => {
      const result = await runDr('add', 'security', 'policy', 'auth-policy',
        '--source-symbol', 'validateAuth',
        '--source-provenance', 'extracted'
      );
    ...
    it('should fail with invalid commit SHA', async () => {
      const result = await runDr('add', 'security', 'policy', 'auth-policy',
        ...
        '--source-provenance', 'extracted',
        '--source-repo-remote', 'https://github.com/example/repo.git',
        '--source-repo-commit', 'not-a-valid-sha'
      );

      expect(result.exitCode).toBe(1);
      expect(result.stderr).toContain('--source-repo-commit must be');
    });
    ...
    it('should support all valid provenance types', async () => {
      ...
      for (let i = 0; i < provenanceTypes.length; i++) {
        const result = await runDr('add', 'security', 'policy', `policy-${i}`,
          '--source-file', 'src/auth/policy.ts',
          '--source-provenance', provenanceTypes[i]
        );
  ...
  describe('update command with source references', () => {
    beforeEach(async () => {
      ...
      await runDr('add', 'security', 'policy', 'auth-policy',
        '--name', 'Auth Policy'
      );
    });
    ...
    it('should fail with invalid commit SHA during update', async () => {
      const result = await runDr('update', 'security-policy-auth-policy',
        ...
        '--source-provenance', 'extracted',
        '--source-repo-remote', 'https://github.com/example/repo.git',
        '--source-repo-commit', 'invalid-sha'
      );
  ...
  describe('show command displaying source references', () => {
    ...
    it('should not display source section when element has no source reference', async () => {
      ...
      await runDr('add', 'security', 'policy', 'no-source-policy',
    ...
    it('should display source reference without symbol when not provided', async () => {
      ...
      await runDr('add', 'security', 'policy', 'simple-policy',
        '--name', 'Simple Policy',
        '--source-file', 'src/policy.ts',
        '--source-provenance', 'manual'
      );
    ...
    it('should display source reference without repository when not provided', async () => {
      ...
      await runDr('add', 'security', 'policy', 'no-repo-policy',
        '--name', 'Policy Without Repo',
        '--source-file', 'src/policy.ts',
        '--source-provenance', 'manual'
      );
  ...
  describe('end-to-end workflow with source references', () => {
    it('should support complete workflow: add, update, and display', async () => {
      ...
      const addResult = await runDr('add', 'application', 'component', 'auth-service',
        '--name', 'Authentication Service',
        '--description', 'Core authentication component',
      ...
      const updateResult = await runDr('update', 'application-component-auth-service',
        ...
        '--source-provenance', 'extracted',
        '--source-repo-remote', 'https://github.com/example/app.git',
        '--source-repo-commit', 'abcdef1234567890abcdef1234567890abcdef12'
      );
