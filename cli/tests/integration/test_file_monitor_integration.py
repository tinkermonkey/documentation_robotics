...
class TestEndToEndIntegration:
    """Test complete end-to-end file monitoring and broadcasting."""

    @pytest.mark.asyncio
    async def test_file_change_broadcasts_to_clients(self, initialized_server, temp_dir):
        """Test file change is detected and broadcast to all clients."""
        ...
        try:
            ...
            layer_dir = model_path / "business"
            layer_dir.mkdir(parents=True, exist_ok=True)
            new_file = layer_dir / "integration-test.yaml"
            new_file.write_text(
                yaml.dump(
                    {
                        "id": "business.service.integration-test",
            ...
            # Wait for file change to be detected and broadcast
            ...
            sent_message = mock_ws.send_json.call_args[0][0]
            assert sent_message["type"] == "element_added"
            assert sent_message["data"]["layer"] == "business"

        finally:
            initialized_server.file_monitor.stop()

    @pytest.mark.asyncio
    async def test_element_data_extracted_correctly(self, initialized_server, temp_dir):
        """Test element data is correctly extracted from file."""
        mock_ws = AsyncMock(spec=web.WebSocketResponse)
        mock_ws.send_json = AsyncMock()
        initialized_server.websockets.add(mock_ws)
        ...
        try:
            # Create file with detailed element data
            model_path = initialized_server.model.model_path
            layer_dir = model_path / "business"
            layer_dir.mkdir(parents=True, exist_ok=True)
            new_file = layer_dir / "detailed-service.yaml"

            element_data = {
                "id": "business.service.detailed",
                "name": "Detailed Service",
                "type": "service",
                "description": "A service with detailed data",
                "properties": {"status": "active", "version": "1.0.0"},
            }

            new_file.write_text(yaml.dump(element_data))

            # Wait for processing
            await asyncio.sleep(0.3)

            # Verify broadcast contains element data
            assert mock_ws.send_json.called
            sent_message = mock_ws.send_json.call_args[0][0]
            assert sent_message["data"]["element"]["name"] == "Detailed Service"
            assert sent_message["data"]["element"]["description"] == "A service with detailed data"
            assert sent_message["data"]["element"]["properties"]["version"] == "1.0.0"

        finally:
            initialized_server.file_monitor.stop()

    @pytest.mark.asyncio
    async def test_deletion_broadcasts_without_element_data(self, initialized_server, temp_dir):
        """Test file deletion broadcasts without element data."""
        mock_ws = AsyncMock(spec=web.WebSocketResponse)
        mock_ws.send_json = AsyncMock()
        initialized_server.websockets.add(mock_ws)
        ...
        layer_dir.mkdir(parents=True, exist_ok=True)
        test_file = layer_dir / "to-delete.yaml"
        test_file.write_text(yaml.dump({"id": "business.service.to-delete", "name": "To Delete"}))

        initialized_server.file_monitor = FileMonitor(
            model_path, initialized_server._handle_file_change, debounce_seconds=0.1
        )
        initialized_server.file_monitor.start()
        ...
        try:
            ...
            # Verify broadcast for deletion
            assert mock_ws.send_json.called
            sent_message = mock_ws.send_json.call_args[0][0]
            assert sent_message["type"] == "element_removed"
            assert sent_message["data"]["layer"] == "business"
            # Element data should be None or not present for deletions
            assert sent_message["data"].get("element") is None

        finally:
            initialized_server.file_monitor.stop()

    @pytest.mark.asyncio
    async def test_multiple_file_changes_broadcast_separately(self, initialized_server, temp_dir):
        """Test multiple file changes are broadcast as separate messages."""
        mock_ws = AsyncMock(spec=web.WebSocketResponse)
        mock_ws.send_json = AsyncMock()
        initialized_server.websockets.add(mock_ws)
        ...
        try:
            ...
            file1.write_text(yaml.dump({"id": "business.service.1", "name": "Service 1"}))

            await asyncio.sleep(0.15)

            file2 = layer_dir / "service2.yaml"
            file2.write_text(yaml.dump({"id": "business.service.2", "name": "Service 2"}))
