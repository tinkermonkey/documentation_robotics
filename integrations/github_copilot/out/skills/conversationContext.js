...
exports.ConversationContextAnalyzer = void 0;
...
class ConversationContextAnalyzer {
     ...
     */
    static analyze(history, currentMessage) {
     ...
     */
    static detectOperations(commands) {
        const operations = [];
        for (const cmd of commands) {
            if (cmd.includes('dr add')) {
                operations.push(types_1.OperationType.ADD_ELEMENT);
            }
            else if (cmd.includes('dr update')) {
                operations.push(types_1.OperationType.MODIFY_ELEMENT);
            }
            else if (cmd.includes('dr remove')) {
                operations.push(types_1.OperationType.DELETE_ELEMENT);
            }
            else if (cmd.includes('dr validate')) {
                operations.push(types_1.OperationType.VALIDATE);
            }
            else if (cmd.includes('dr ingest') || cmd.includes('dr extract')) {
                operations.push(types_1.OperationType.EXTRACT_CODE);
            }
            else if (cmd.includes('dr changeset apply')) {
                operations.push(types_1.OperationType.APPLY_CHANGESET);
            }
        }
        return operations;
    }
     ...
     */
    static extractModifiedLayers(commands) {
        const layers = [];
     ...
     */
    static detectActiveChangeset(commands, responseText) {
        ...
        const hasApplyOrDiscard = commands.some(cmd => cmd.includes('dr changeset apply') || cmd.includes('dr changeset discard'));
...
exports.ConversationContextAnalyzer = ConversationContextAnalyzer;
