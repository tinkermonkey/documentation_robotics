"""Markdown layer specification validator module.

This module provides the MarkdownValidator class for validating markdown layer specifications.
"""
...
from generators.markdown_parser import MarkdownLayerParser
...
class MarkdownValidator:
    ...
    def __init__(self, root_dir: Optional[Path] = None):
        ...
        """
        if root_dir is None:
            # Auto-detect: validation module is in scripts/validation, root is 2 levels up
            import sys
            script_dir = Path(sys.path[0]) if sys.path else Path(__file__).parent.parent
            root_dir = script_dir.parent if 'scripts' in str(script_dir) else script_dir
